...
namespace Irony.Samples
{
	...
	public class WikiCodeplexGrammar : Grammar, ICanRunSample
	{
		...
		{
			this.GrammarComments = @"A grammar for reading codeplex wiki files and transforming them into HTML
...
";
			...
			wikiText.Rule = this.MakeStarRule(wikiText, wikiElement);

			this.Root = wikiText;
			this.MarkTransient(wikiElement);

			// We need to clear punctuation flag on NewLine, so it is not removed from parse tree
			this.NewLine.SetFlag(TermFlags.IsPunctuation, false);
