namespace Irony.Parsing
{
	...
	public class ErrorRecoveryParserAction : ParserAction
	{
		...
		protected bool TryRecoverFromError(ParsingContext context)
		{
			...
			// 3. Now we need to go along error production until the end, shifting tokens that CAN be shifted and ignoring others.
		...
		private ParserAction FindErrorShiftActionInStack(ParsingContext context)
		{
			...
			while (context.ParserStack.Count >= 1)
			{
				...
				if (context.CurrentParserState.Actions.TryGetValue(grammar.SyntaxError, out errorShiftAction) && errorShiftAction is ShiftParserAction)
